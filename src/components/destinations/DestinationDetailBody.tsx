...
export function DestinationDetailBody({
  ...
  return (
    ...
    >
      ...
      <motion.dl
        variants={item}
        className="mt-8 grid grid-cols-2 gap-4 rounded-2xl bg-saffron-50/70 p-5 ring-1 ring-saffron-100 sm:grid-cols-3"
      >
        <div>
          ...
          <dd className="mt-1 font-medium text-ink-900">{destination.bestTime}</dd>
        </div>
        <div>
          <dt className="flex items-center gap-1.5 text-[11px] uppercase tracking-[0.18em] text-ink-500">
            <Clock className="h-3.5 w-3.5" /> Duration
          </dt>
          <dd className="mt-1 font-medium text-ink-900">{destination.duration}</dd>
        </div>
        <div className="col-span-2 sm:col-span-1">
          ...
          <dd className="mt-1 font-medium text-ink-900">
            ...
            <span className="text-xs font-normal text-ink-500"> / group</span>
          </dd>
        </div>
      </motion.dl>
      ...
      <motion.div variants={item} className="mt-10">
        <h3 className="font-display text-2xl text-ink-900">Trip highlights</h3>
        <ul className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2">
          {destination.highlights.map((h) => (
            <li key={h} className="flex items-start gap-3">
              <span className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-saffron-100 text-saffron-700">
      ...
      <motion.div variants={item} className="mt-10">
        ...
        <div className="mt-3 flex flex-wrap gap-1.5">
          {destination.locations.map((loc) => (
            <span
              key={loc}
              className="rounded-full bg-white px-3 py-1.5 text-xs font-medium text-saffron-800 ring-1 ring-saffron-100"
            >
              {loc}
            </span>
          ))}
